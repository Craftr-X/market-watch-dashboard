...
def refresh_market(store: MarketStore) -> dict:
    ...
    return {"status": "ok", "date": payload["date"], "updated_at": updated_at}


def create_app(database_url: str | None = None) -> FastAPI:
    app = FastAPI(title="A股每日行情与强势观察", version="0.1.0")
    app.add_middleware(
        CORSMiddleware,
    ...
    def ensure_data() -> tuple[str | None, str | None]:
        ...
        if not rows:
            ...
            return result["updated_at"], "sample-akshare-compatible"
    ...
    def refresh_job():
        return envelope(refresh_market(store), None, "sample-akshare-compatible")
